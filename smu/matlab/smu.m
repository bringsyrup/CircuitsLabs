                ...
                if (ch==1) || (ch==2)
                    if (units<0) || (units>1)
                        ...
                        if range~=obj.OVER_RANGE
                            ...
                            ret = calllib('usb', 'control_transfer', obj.dev, 64, obj.SET_SRC, temp, 256*uint16(units)+uint16(ch), 0, obj.buffer);
                            if ret<0
...
%%         can be 1 or 2.
%%
        function adc = get_adc(obj, ch)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    ret = calllib('usb', 'control_transfer', obj.dev, 192, obj.GET_ADC_KILL60HZ, 0, uint16(ch), 2, obj.buffer);
                    if ret<0
                        fprintf('Unable to send GET_ADC_KILL60HZ vendor request.\n');
                    else
                        adc = double(256*uint16(obj.buffer.value(2))+uint16(obj.buffer.value(1)));
                    end
                else
                    fprintf('Illegal channel number specified.\n');
                end
...
%%         can be 1 or 2.
%%
        function ref = get_ref(obj, ch)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    ret = calllib('usb', 'control_transfer', obj.dev, 192, obj.GET_REF, 0, uint16(ch), 2, obj.buffer);
                    if ret<0
                        fprintf('Unable to send GET_REF vendor request.\n');
                    else
                        ref = 256*uint16(obj.buffer.value(2))+uint16(obj.buffer.value(1));
                    end
                else
                    fprintf('Illegal channel number specified.\n');
                end
...
%%
        function set_ref(obj, ch, ref)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    if (ref<0) || (ref>4095)
                        fprintf('Illegal channel reference specified.\n');
                    else
                        ret = calllib('usb', 'control_transfer', obj.dev, 64, obj.SET_REF, uint16(ref), uint16(ch), 0, obj.buffer);
                        if ret<0
                            fprintf('Unable to send SET_REF vendor request.\n');
                        end
                    end
                else
                    fprintf('Illegal channel number specified.\n');
...
%%
        function save_ref(obj, ch)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    ret = calllib('usb', 'control_transfer', obj.dev, 64, obj.SAVE_REF, 0, uint16(ch), 0, obj.buffer);
                    if ret<0
                        fprintf('Unable to send SAVE_REF vendor request.\n');
                    end
                else
                    fprintf('Illegal channel number specified.\n');
                end
...
%%         EEPROM, where CH can be 1 or 2.
%%
        function load_ref(obj, ch)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    ret = calllib('usb', 'control_transfer', obj.dev, 64, obj.LOAD_REF, 0, uint16(ch), 0, obj.buffer);
                    if ret<0
                        fprintf('Unable to send LOAD_REF vendor request.\n');
                    end
                else
                    fprintf('Illegal channel number specified.\n');
                end
